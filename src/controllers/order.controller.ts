 ...
 */
export const assignPersonnelController = async (
  req: AuthenticatedRequest,
  res: Response
) => {
  try {
    ...
    const { orderId } = req.params;
    const { role, userId, name, contact, replace } = req.body as {
      role?: string;
      userId?: string;
      name?: string;
      contact?: string;
    ...
    if (replace) {
      order.assigned_personnel = order.assigned_personnel.filter(
        (p: any) => String(p.userId) !== String(userId) && p.role !== role
      );
    }

    order.assigned_personnel.push(assigned as any);
